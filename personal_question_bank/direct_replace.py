...
def clear_and_replace():
    """清除并替换为LeetCode题目"""
    print("=== 直接替换为LeetCode题目 ===\n")
    
    with app.app_context():
        # 1. 清除所有相关数据
        print("正在清除现有数据...")
        db.session.execute(text("DELETE FROM similar_questions"))
        db.session.execute(text("DELETE FROM wrong_questions"))
        db.session.execute(text("DELETE FROM learning_records"))
        db.session.execute(text("DELETE FROM user_knowledge_stats"))
        db.session.execute(text("DELETE FROM questions"))
        db.session.execute(text("DELETE FROM knowledge_points"))
        db.session.commit()
        print("✅ 现有数据已清除")
        ...
        knowledge_points_data = [
            ("数组", "数据结构", "数组操作、双指针技巧", 1, "academic"),
            ("链表", "数据结构", "链表操作、快慢指针", 2, "academic"),
            ("字符串", "数据结构", "字符串处理、模式匹配", 2, "academic"),
            ("哈希表", "数据结构", "哈希表应用、查找优化", 2, "academic"),
            ("滑动窗口", "算法", "窗口技巧、子串问题", 3, "academic"),
            ("二分查找", "算法", "有序查找、分治思想", 3, "academic"),
        ]
        
        knowledge_point_ids = {}
        for idx, (name, category, description, difficulty_level, question_bank_mode) in enumerate(knowledge_points_data, 1):
            db.session.execute(text("""
                INSERT INTO knowledge_points (id, name, category, description, difficulty_level, question_bank_mode, created_at)
                VALUES (:id, :name, :category, :description, :difficulty_level, :question_bank_mode, :created_at)
            """), {
                'id': idx,
                'name': name,
                'category': category,
                ...
                'question_bank_mode': question_bank_mode,
                'created_at': datetime.utcnow()
            })
            knowledge_point_ids[name] = idx
...
- 正确处理正负数的取模和整除""",
        ...
        for problem_data in leetcode_problems:
            knowledge_point_id = knowledge_point_ids.get(problem_data["knowledge_point"])
            
            # 直接使用SQL插入，只使用存在的字段
            db.session.execute(text("""
                INSERT INTO questions (
                    title, content, question_type, difficulty, estimated_time, 
                    knowledge_point_id, correct_answer, explanation, 
                    programming_language, test_cases, external_platform, 
                    external_id, question_bank_mode, created_at
                ) VALUES (
                    :title, :content, :question_type, :difficulty, :estimated_time, 
                    :knowledge_point_id, :correct_answer, :explanation, 
                    :programming_language, :test_cases, :external_platform, 
